	...
	throw(runtime_error)
{
    ...
    if((r != -1) & (g != -1) & (b != -1))
    {
        SDL_SetColorKey(p_temp_image, SDL_TRUE, SDL_MapRGB(p_temp_image->format, static_cast<Uint8>(r), static_cast<Uint8>(g), static_cast<Uint8>(b)));
    }
	...
	//gluBuild2DMipmaps(GL_TEXTURE_2D, format, p_bitmap->w, p_bitmap->h,	GL_RGBA,
	//	GL_UNSIGNED_BYTE, p_bitmap->pixels);
